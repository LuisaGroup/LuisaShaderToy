...
int main(int argc, char *argv[]) {
    ...
    auto axis = float3(1.0f, 1.0f, 0.0f);
    auto tgt = float3(1.0f, 1.7f, 1.1f);
    auto cpos = tgt + axis;
    auto vel = 0.2f * axis;
    auto key_G = 71.5f/256.0f;


    Callable hash = [](Float3 x) {
        return fract(87.3f * dot(x, make_float3(0.1f, 0.9f, 0.7f)));
    };
    ...
    Callable round2 = [](Float3 x, Float3 a) {
        return 2.0f * floor(0.5f * (x + float3(1.0f) - a));
    };
    ...
    /*Callable make_float4x4 = [](Float4 c0, Float4 c1, Float4 c2, Float4 c3) {
        Float4x4 m{c0, c1, c2, c3};
    return m;
    }
    */

    Callable permute = [](Float3 e, Float3 f, Float3 g, Float3 h, Float p) {
	return ite(p < i, make_float4x4(make_float4(e,1.0f), make_float4(f,1.0f), make_float4(g, 1.0f), make_float4(h, 1.0f)),
    ...
    // auto mod = [](auto &&x, auto &&y) { return x - y * floor(x / y); };


    Callable map = [&round2, &permute, &pselect, &pdist, &torus, &opU, &hash, &hueOf, &randomPerp, &randomBasis, &vel]
                                                            (Float3 pos, Float iTime) {
        ...
        /*if (texture(iChannel0, make_float2(key_G, 0.75f)).x > 0.0f) {
            ...
            vec2 grid = make_float2(dline, 2.0f);
    ...
    Callable hue = [](Float h) {
        Var c = mod(h*6.0f + make_float3(2.0f, 0.0f, 4.0f), 6.0f);
    ...
    Callable calcNormal = [&map](Float3 pos, Float iTime ){
	    Var eps = make_float3(0.0001f, 0.0f, 0.0f);
	    Var nor = make_float3(
    ...
    Callable mainImage = [&](Float2 fragCoord, Float2 iResolution, Float iTime, Float4 iMouse) {
        ...
        Var uv = (fragCoord.xy() - 0.5f*iResolution.xy()) * yscl / iResolution.y;
        Var up = make_float3(0.0f, 1.0f, 0.0f);
        ...
        Var color = make_float3(shade(ro, rd, iTime));
